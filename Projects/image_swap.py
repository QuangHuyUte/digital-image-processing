...
# =========================================================
#           FREQUENCY DOMAIN HELPERS (FFT trên kênh Y)
# =========================================================
def _fft_spectrum_image(y):
    """Trả về ảnh magnitude spectrum (log) normalised [0,255]."""
    f = np.fft.fft2(y.astype(np.float32))
...
def _freq_filter_y(y, mode, D0, D1, n, alpha):
    """
    Áp frequency filter lên kênh Y (grayscale).
    mode: string tên filter trong OptionMenu.
    D0, D1: cutoffs
    n: Butterworth order
    alpha: hệ số Laplacian sharpen trong miền tần số
    """
    if mode == "None":
        return y
    ...
    V, U = np.meshgrid(v, u)
    D = np.sqrt(U ** 2 + V ** 2) + 1e-8  # tránh chia 0

    mode = mode.strip()
    H = None

    # --- Ideal / Gaussian / Butterworth basic ---
    if mode == "Ideal Low-pass":
        H = (D <= D0).astype(np.float32)

    elif mode == "Ideal High-pass":
        H = (D > D0).astype(np.float32)

    elif mode == "Gaussian Low-pass":
        H = np.exp(-(D ** 2) / (2 * (D0 ** 2)))

    elif mode == "Gaussian High-pass":
        H = 1.0 - np.exp(-(D ** 2) / (2 * (D0 ** 2)))

    elif mode == "Butterworth Low-pass":
        H = 1.0 / (1.0 + (D / D0) ** (2 * n))

    elif mode == "Butterworth High-pass":
        H = 1.0 - 1.0 / (1.0 + (D / D0) ** (2 * n))

    # --- Band filters ---
    elif mode == "Ideal Band-pass":
        H = np.logical_and(D >= D0, D <= D1).astype(np.float32)

    elif mode == "Ideal Band-stop":
        H = np.logical_or(D < D0, D > D1).astype(np.float32)

    elif mode == "Butterworth Band-pass":
        # HP * LP
        lp = 1.0 / (1.0 + (D / D1) ** (2 * n))
        hp = 1.0 - 1.0 / (1.0 + (D / D0) ** (2 * n))
        H = hp * lp

    elif mode == "Butterworth Band-stop":
        lp = 1.0 / (1.0 + (D / D1) ** (2 * n))
        hp = 1.0 - 1.0 / (1.0 + (D / D0) ** (2 * n))
        bandpass = hp * lp
        H = 1.0 - bandpass

    # --- Laplacian Sharpen trong miền tần số ---
    elif mode == "Laplacian Sharpen":
        # H_Lap(u,v) ~ -(u^2 + v^2)
        H_lap = -(D ** 2)
        H_lap_norm = H_lap / (np.max(np.abs(H_lap)) + 1e-8)
        fshift_filt = fshift + alpha * H_lap_norm * fshift
        f_ishift = np.fft.ifftshift(fshift_filt)
        img_back = np.fft.ifft2(f_ishift)
        y_new = np.abs(img_back)
        return np.clip(y_new, 0, 255).astype(np.uint8)

    # Nếu không set được H thì trả về y
    if H is None:
        return y
...
class DemoApp(ctk.CTk):
    ...
    def _build_controls(self):
        ...
        # ---------------- FREQUENCY DOMAIN ----------------
        self._header("FREQUENCY DOMAIN – FFT (kênh Y)", "#009688")

        # Bật / tắt frequency filter
        self.freq_enable = ctk.BooleanVar(value=False)
        ctk.CTkSwitch(
            self.scroll,
            text="Bật Frequency Filters",
            variable=self.freq_enable,
            command=self._on_freq_changed,
        ).pack(anchor="w", pady=(0, 4))

        # Loại filter
        self.freq_type = ctk.StringVar(value="None")
        ctk.CTkLabel(
            self.scroll, text="Loại frequency filter:",
        ...
        self.freq_type_menu = ctk.CTkOptionMenu(
            self.scroll,
            variable=self.freq_type,
            values=filter_values,
            command=lambda _val: self._on_freq_changed(),
        )
        self.freq_type_menu.pack(fill="x", padx=4, pady=(0, 6))

        # --- Sliders riêng cho frequency (D0, D1, order n, alpha) ---

        # D0
        wrap_d0 = ctk.CTkFrame(self.scroll, fg_color="transparent")
        wrap_d0.pack(fill="x", pady=2)
        row_d0 = ctk.CTkFrame(wrap_d0, fg_color="transparent")
        row_d0.pack(fill="x")
        self.freq_D0_label_text = "Freq: D0 (cutoff thấp)"
        lbl_d0 = ctk.CTkLabel(row_d0, text=self.freq_D0_label_text,
                              font=ctk.CTkFont(size=11))
        lbl_d0.pack(side="left")
        self.freq_D0_val = ctk.DoubleVar(value=60)
        lbl_d0_val = ctk.CTkLabel(row_d0, text="60",
                                  font=ctk.CTkFont(size=11))
        lbl_d0_val.pack(side="right")
        slider_d0 = ctk.CTkSlider(
            wrap_d0,
            from_=5,
            to=400,
            number_of_steps=395,
            command=lambda v: self._on_freq_slider_change(self.freq_D0_val,
                                                          lbl_d0_val, v),
        )
        slider_d0.set(60)
        slider_d0.pack(fill="x", padx=2, pady=(2, 6))
        self.freq_D0_label = lbl_d0
        self.freq_D0_slider = slider_d0

        # D1
        wrap_d1 = ctk.CTkFrame(self.scroll, fg_color="transparent")
        wrap_d1.pack(fill="x", pady=2)
        row_d1 = ctk.CTkFrame(wrap_d1, fg_color="transparent")
        row_d1.pack(fill="x")
        self.freq_D1_label_text = "Freq: D1 (band high)"
        lbl_d1 = ctk.CTkLabel(row_d1, text=self.freq_D1_label_text,
                              font=ctk.CTkFont(size=11))
        lbl_d1.pack(side="left")
        self.freq_D1_val = ctk.DoubleVar(value=120)
        lbl_d1_val = ctk.CTkLabel(row_d1, text="120",
                                  font=ctk.CTkFont(size=11))
        lbl_d1_val.pack(side="right")
        slider_d1 = ctk.CTkSlider(
            wrap_d1,
            from_=5,
            to=400,
            number_of_steps=395,
            command=lambda v: self._on_freq_slider_change(self.freq_D1_val,
                                                          lbl_d1_val, v),
        )
        slider_d1.set(120)
        slider_d1.pack(fill="x", padx=2, pady=(2, 6))
        self.freq_D1_label = lbl_d1
        self.freq_D1_slider = slider_d1

        # Butterworth order n
        wrap_n = ctk.CTkFrame(self.scroll, fg_color="transparent")
        wrap_n.pack(fill="x", pady=2)
        row_n = ctk.CTkFrame(wrap_n, fg_color="transparent")
        row_n.pack(fill="x")
        self.freq_n_label_text = "Butterworth order n"
        lbl_n = ctk.CTkLabel(row_n, text=self.freq_n_label_text,
                             font=ctk.CTkFont(size=11))
        lbl_n.pack(side="left")
        self.freq_n_val = ctk.DoubleVar(value=2)
        lbl_n_val = ctk.CTkLabel(row_n, text="2",
                                 font=ctk.CTkFont(size=11))
        lbl_n_val.pack(side="right")
        slider_n = ctk.CTkSlider(
            wrap_n,
            from_=1,
            to=5,
            number_of_steps=4,
            command=lambda v: self._on_freq_slider_change(self.freq_n_val,
                                                          lbl_n_val, v, True),
        )
        slider_n.set(2)
        slider_n.pack(fill="x", padx=2, pady=(2, 6))
        self.freq_n_label = lbl_n
        self.freq_n_slider = slider_n

        # Laplacian alpha
        wrap_a = ctk.CTkFrame(self.scroll, fg_color="transparent")
        wrap_a.pack(fill="x", pady=2)
        row_a = ctk.CTkFrame(wrap_a, fg_color="transparent")
        row_a.pack(fill="x")
        self.freq_alpha_label_text = "Freq Laplacian α"
        lbl_a = ctk.CTkLabel(row_a, text=self.freq_alpha_label_text,
                             font=ctk.CTkFont(size=11))
        lbl_a.pack(side="left")
        self.freq_alpha_val = ctk.DoubleVar(value=1.5)
        lbl_a_val = ctk.CTkLabel(row_a, text="1.50",
                                 font=ctk.CTkFont(size=11))
        lbl_a_val.pack(side="right")
        slider_a = ctk.CTkSlider(
            wrap_a,
            from_=0.1,
            to=5.0,
            number_of_steps=49,
            command=lambda v: self._on_freq_slider_change(self.freq_alpha_val,
                                                          lbl_a_val, v),
        )
        slider_a.set(1.5)
        slider_a.pack(fill="x", padx=2, pady=(2, 6))
        self.freq_alpha_label = lbl_a
        self.freq_alpha_slider = slider_a

        # Xem FFT
        self.fft_view_on = ctk.BooleanVar(value=False)
        ctk.CTkSwitch(
            self.scroll,
            text="Xem Spectrum FFT (magnitude)",
            variable=self.fft_view_on,
            command=lambda: self._schedule_preview(0),
        ).pack(anchor="w", pady=(4, 6))

        # Cập nhật trạng thái lock/unlock
        self._update_freq_widgets_state()

    # ---------- Frequency helpers (UI) ----------
    def _on_freq_slider_change(self, var, lbl, value, integer=False):
        if integer:
    ...
    def _update_freq_widgets_state(self):
        enabled = self.freq_enable.get()
        mode = self.freq_type.get()

        use_D0 = enabled and (mode not in ["None", "Laplacian Sharpen"])
        use_D1 = enabled and ("Band-pass" in mode or "Band-stop" in mode)
        use_order = enabled and ("Butterworth" in mode)
        use_alpha = enabled and (mode == "Laplacian Sharpen")

        # helper: gán text + icon 🔒 và state slider
        def lock(label, slider, base_text, use):
            if slider is None or label is None:
                return
            slider.configure(state="normal" if use else "disabled")
            label.configure(
                text=base_text if use else (base_text + "  🔒")
            )

        lock(self.freq_D0_label, self.freq_D0_slider,
             self.freq_D0_label_text, use_D0)
        lock(self.freq_D1_label, self.freq_D1_slider,
             self.freq_D1_label_text, use_D1)
        lock(self.freq_n_label, self.freq_n_slider,
             self.freq_n_label_text, use_order)
        lock(self.freq_alpha_label, self.freq_alpha_slider,
             self.freq_alpha_label_text, use_alpha)
    ...
    def reset_params(self):
        ...
        # Frequency
        self.freq_enable.set(False)
        self.freq_type.set("None")
        self.freq_D0_val.set(60)
        self.freq_D1_val.set(120)
        self.freq_n_val.set(2)
        self.freq_alpha_val.set(1.5)
        self.fft_view_on.set(False)
        self._update_freq_widgets_state()
    ...
    def _run_pipeline(self, src_bgr):
        ...
        # 6. FREQUENCY FILTERS (trên kênh Y, sau spatial)
        if self.freq_enable.get() and self.freq_type.get() != "None":
            ycrcb = cv2.cvtColor(out, cv2.COLOR_BGR2YCrCb)
            y, cr, cb = cv2.split(ycrcb)
            D0 = float(self.freq_D0_val.get())
            D1 = float(self.freq_D1_val.get())
            n = int(round(float(self.freq_n_val.get())))
            alpha = float(self.freq_alpha_val.get())
            y2 = _freq_filter_y(y, self.freq_type.get(), D0, D1, n, alpha)
            out = cv2.cvtColor(cv2.merge([y2, cr, cb]), cv2.COLOR_YCrCb2BGR)

        # 7. XEM FFT SPECTRUM CỦA KẾT QUẢ SAU KHI LỌC
        if self.fft_view_on.get():
            ycrcb = cv2.cvtColor(out, cv2.COLOR_BGR2YCrCb)
            y, _, _ = cv2.split(ycrcb)
            mag = _fft_spectrum_image(y)
